...
export default async function InvoicePage({ params }: InvoicePageProps) {
    ...
    const invoiceNumber = decodeURIComponent(resolvedParams.invoiceNumber.join('/'));
        ...
        <div className="min-h-screen bg-gray-100 p-4 md:p-8 print:p-0 print:bg-white flex justify-center">
            ...
            <div className="bg-white shadow-lg w-full max-w-[210mm] min-h-[297mm] p-8 md:p-12 relative overflow-hidden print:shadow-none print:w-full">
                ...
                <div className="relative z-10 mt-12 pt-8 border-t border-slate-100">
                    <h4 className="font-bold text-slate-500 mb-4 text-xs tracking-wider uppercase">PAYMENT INSTRUCTIONS</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {(await prisma.bankAccount.findMany({ where: { isActive: true } })).map(bank => (
                            <div key={bank.id} className="bg-slate-50 p-4 rounded-lg flex items-center justify-between border border-slate-100">
                                <div>
                                    <div className="font-bold text-slate-900">{bank.bankName}</div>
                                    <div className="font-mono text-lg tracking-wider text-slate-700">{bank.accountNumber}</div>
                                    <div className="text-xs text-slate-500 uppercase">a.n {bank.accountHolder}</div>
                                </div>
                                {bank.logo && <img src={bank.logo} className="h-8 object-contain opacity-80" alt={bank.bankName} />}
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* Footer Terms */}
                <div className="relative z-10 mt-8 pt-8 border-t border-slate-100 text-xs text-slate-400 space-y-1">
