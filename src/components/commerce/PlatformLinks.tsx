...
export default function PlatformLinks({ links }: { links: ExternalLinks }) {
    ...
    const platforms = [
        ...
        // { key: 'tokopedia', label: 'Tokopedia', icon: '🛒', color: 'hover:bg-[#42B549]/5 hover:border-[#42B549]/20', textColor: 'text-[#42B549]' },
    ];
                ...
                {platforms.map((p) => {
                    ...
                    return (
                        <a
                            ...
                            className={`flex items-center gap-3 px-6 py-4 bg-white border border-[#E5E1D8] rounded-[1.5rem] transition-all duration-300 group shadow-sm hover:shadow-md ${p.color}`}
                        >
                            <div className="w-12 h-12 flex items-center justify-center relative flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
                                {(p as any).logoSrc ? (
                                    <img
                                    ...
                                    <span className="text-2xl">{(p as any).icon}</span>
                                )}
                            </div>
